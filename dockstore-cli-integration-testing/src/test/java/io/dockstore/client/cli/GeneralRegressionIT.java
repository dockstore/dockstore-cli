...
public class GeneralRegressionIT extends BaseIT {
    ...
    public void testVersionTagWDLCWLAndDockerfilePathsAlterationOldClient() {
        ...
        final long count2 = testingPostgres.runSelectStatement(
            "select count(*) from tag,tool where tool.registry = '" + Registry.QUAY_IO.toString()
                + "' and tool.namespace = 'dockstoretestuser2' and tool.name = 'quayandgithub' and tool.toolname = '' and tool.id=tag.parentid and tag.valid = 'f'",
            long.class);
    ...
    public void testVersionTagWDLOldClient() {
        ...
        final long count = testingPostgres.runSelectStatement(
            "select count(*) from tag,tool where tool.registry = '" + Registry.QUAY_IO.toString()
                + "' and tool.namespace = 'dockstoretestuser2' and tool.name = 'quayandgithubwdl' and tool.toolname IS NULL and tool.id=tag.parentid and tag.valid = 'f'",
            long.class);
